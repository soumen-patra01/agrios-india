/* Service marketplace database — separate IndexedDB so the Supabase swap
   is independent from the product marketplace (agrios-marketplace). */

const DB_NAME = "agrios-svc-marketplace";
const DB_VERSION = 1;

const STORES = {
  providers:    ["type", "status"],
  services:     ["providerId", "category", "status"],
  bookings:     ["serviceId", "providerId", "status"],
  svcReviews:   ["serviceId", "providerId"],
  availability: ["providerId", "dayOfWeek"],
};
